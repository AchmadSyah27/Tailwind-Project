<!DOCTYPE html>
<html lang="en" className="scroll-smooth">
	<head>
		<meta charset="UTF-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1.0" />
		<title>Cohee - Detail</title>

		<!-- Tailwind CSS CDN -->
		<script src="https://cdn.tailwindcss.com"></script>

		<!-- Custom Tailwind Config -->
		<script>
			tailwind.config = {
				theme: {
					extend: {
						colors: {
							primary: "#433738",
							secondary: "#c49b63",
							cream: "#dfcda9",
							white: "#f8f7f7",
							black: "#000",
							teal: "#3d8d7a",
							accent: "#ff6600",
						},
						borderRadius: {
							DEFAULT: "0.5rem",
						},
						fontFamily: {
							"big-shoulders": ['"Big Shoulders"', "sans-serif"],
							roboto: ["Roboto", "sans-serif"],
						},
					},
				},
			};
		</script>

		<script src="https://unpkg.com/feather-icons"></script>
		<link rel="stylesheet" href="./assets/css/style.css" />

		<link
			rel="stylesheet"
			href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css"
		/>
		<link rel="preconnect" href="https://fonts.googleapis.com" />
		<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
		<link
			href="https://fonts.googleapis.com/css2?family=Big+Shoulders:opsz,wght@10..72,100..900&family=Roboto:ital,wght@0,100..900;1,100..900&display=swap"
			rel="stylesheet"
		/>
		<!-- <link rel="stylesheet" href="./assets/css/style.css"> -->

		<!-- swiper js -->
		<link
			rel="stylesheet"
			href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css"
		/>
	</head>
	<body className="font-roboto leading-relaxed bg-primary text-black">
		<main>
			<!-- Navbar - Hendri -->
			<header className="sticky top-0 z-50 w-full bg-primary shadow-md">
				<div
					className="container mx-auto px-4 flex items-center justify-between h-16"
				>
					<a href="" className="block">
						<img
							src="./assets/images/coffee-logo.svg"
							alt="Logo brand"
							className="w-[4.5rem] h-auto"
						/>
					</a>
					<nav className="hidden md:flex items-center gap-8">
						<a
							href="index.html"
							className="text-lg font-medium text-white hover:text-secondary transition-all duration-300"
							>Home</a
						>
						<a
							href="about.html"
							className="text-lg font-medium text-white hover:text-secondary transition-all duration-300"
							>About</a
						>
						<a
							href="blogs.html"
							className="text-lg font-medium text-white hover:text-secondary transition-all duration-300"
							>Blog</a
						>
						<a
							href="menu.html"
							className="text-lg font-medium text-white hover:text-secondary transition-all duration-300"
							>Menu</a
						>
					</nav>
					<div className="flex gap-4 items-center">
						<div className="hamburger md:hidden cursor-pointer">
							<span
								className="bar block w-[25px] h-[3px] my-[5px] mx-auto bg-white transition-all duration-300"
							></span>
							<span
								className="bar block w-[25px] h-[3px] my-[5px] mx-auto bg-white transition-all duration-300"
							></span>
							<span
								className="bar block w-[25px] h-[3px] my-[5px] mx-auto bg-white transition-all duration-300"
							></span>
						</div>
						<div
							className="hamburger__menu fixed left-[-100%] top-16 flex-col bg-primary w-full text-center transition-all duration-300 shadow-lg py-8 z-[99]"
						>
							<a
								href="index.html"
								className="block py-6 text-white text-xl hover:text-secondary transition-all duration-300"
								>Home</a
							>
							<a
								href="about.html"
								className="block py-6 text-white text-xl hover:text-secondary transition-all duration-300"
								>About</a
							>
							<a
								href="blogs.html"
								className="block py-6 text-white text-xl hover:text-secondary transition-all duration-300"
								>Blog</a
							>
							<a
								href="menu.html"
								className="block py-6 text-white text-xl hover:text-secondary transition-all duration-300"
								>Menu</a
							>
						</div>
					</div>
				</div>
			</header>

			<!-- Header Section -->
			<section id="detail__header">
				
			</section>

			<!-- Body Section -->
			<section id="detail__body" className="bg-white py-8 md:py-12 lg:py-16">
				<h2 className="text-3xl md:text-4xl font-bold pb-28 mb-12 text-primary text-center">Menu Kami</h2>
				<div className="container my-auto mx-auto px-4 py-4 flex flex-col md:flex-row justify-center items-center gap-8 ">
						<div className="px-5 pb-5">
							<!--card product-->
							<div className="relative bg-primary rounded-3xl pt-12 mb-24 p-4 max-w-xl w-48 h-auto text-white z-0">
								<!-- Kotak silver -->
								<div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-cream w-28 h-28 rounded-2xl z-20"></div>
								<!-- Gambar gelas -->
								<img src="assets/images/latte.png" alt="latte" className="absolute -top-10 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-auto object-contain z-30" />
								<!-- Konten Produk -->
								<div className="text-center mt-8">
									<h3 className="text-lg font-semibold">Latte</h3>
									<p className="text-sm text-gray-300 mt-1">
									20% espresso, 40% susu skim dan terdapat foam supaya rasa kopi bisa lebih stabil.
									</p>
								</div>
								<!-- Harga + Tombol -->
								<div className="flex justify-between items-center mt-4">
									<button className="rounded mx-1">
										<span className="mt-4 inline-flex items-center gap-2 px-5 py-2 bg-cream text-black font-medium rounded-lg shadow-md hover:bg-white hover:text-black transition">Rp. 35.000</span>
									</button>
								</div>
							</div>
							<!--card product baru-->
							<div className="flex items-center mt-2.5 mb-5">
								<div className="flex items-center space-x-1 rtl:space-x-reverse">
								</div>
							</div>
						</div>
						<div className="px-5 pb-5">
							<!--card product-->
							<div className="relative bg-primary rounded-3xl pt-12 mb-24 p-4 max-w-xl w-48 h-auto text-white z-0">
								<!-- Kotak silver -->
								<div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-cream w-28 h-28 rounded-2xl z-20"></div>
								<!-- Gambar gelas -->
								<img src="assets/images/moccachino.png" alt="moccachino" className="absolute -top-10 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-auto object-contain z-30" />
								<!-- Konten Produk -->
								<div className="text-center mt-8">
									<h3 className="text-lg font-semibold">Moccachino</h3>
									<p className="text-sm text-gray-300 mt-1">
									20% espresso, 40% susu skim dan terdapat foam supaya rasa kopi bisa lebih stabil.
									</p>
								</div>
								<!-- Harga + Tombol -->
								<div className="flex justify-between items-center mt-4">
									<button className="rounded mx-1">
										<span className="mt-4 inline-flex items-center gap-2 px-5 py-2 bg-cream text-black font-medium rounded-lg shadow-md hover:bg-white hover:text-black transition">Rp. 35.000</span>
									</button>
								</div>
							</div>
							<!--card product baru-->
							<div className="flex items-center mt-2.5 mb-5">
								<div className="flex items-center space-x-1 rtl:space-x-reverse">
								</div>
							</div>
						</div>
						<div className="px-5 pb-5">
							<!--card product-->
							<div className="relative bg-primary rounded-3xl pt-12 mb-24 p-4 max-w-xl w-48 h-auto text-white z-0">
								<!-- Kotak silver -->
								<div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-cream w-28 h-28 rounded-2xl z-20"></div>
								<!-- Gambar gelas -->
								<img src="assets/images/frappucino.png" alt="frappucino" className="absolute -top-10 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-auto object-contain z-30" />
								<!-- Konten Produk -->
								<div className="text-center mt-8">
									<h3 className="text-lg font-semibold">Frappucino</h3>
									<p className="text-sm text-gray-300 mt-1">
									20% espresso, 40% susu skim dan terdapat foam supaya rasa kopi bisa lebih stabil.
									</p>
								</div>
								<!-- Harga + Tombol -->
								<div className="flex justify-between items-center mt-4">
									<button className="rounded mx-1">
										<span className="mt-4 inline-flex items-center gap-2 px-5 py-2 bg-cream text-black font-medium rounded-lg shadow-md hover:bg-white hover:text-black transition">Rp. 35.000</span>
									</button>
								</div>
							</div>
							<!--card product baru-->
							<div className="flex items-center mt-2.5 mb-5">
								<div className="flex items-center space-x-1 rtl:space-x-reverse">
								</div>
							</div>
						</div>
						<div className="px-5 pb-5">
							<!--card product-->
							<div className="relative bg-primary rounded-3xl pt-12 mb-24 p-4 max-w-xl w-48 h-auto text-white z-0">
								<!-- Kotak silver -->
								<div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-cream w-28 h-28 rounded-2xl z-20"></div>
								<!-- Gambar gelas -->
								<img src="assets/images/espresso.png" alt="espresso" className="absolute -top-10 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-48 h-auto object-contain z-30" />
								<!-- Konten Produk -->
								<div className="text-center mt-8">
									<h3 className="text-lg font-semibold">Espresso</h3>
									<p className="text-sm text-gray-300 mt-1">
									20% espresso, 40% susu skim dan terdapat foam supaya rasa kopi bisa lebih stabil.
									</p>
								</div>
								<!-- Harga + Tombol -->
								<div className="flex justify-between items-center mt-4">
									<button className="rounded mx-1">
										<span className="mt-4 inline-flex items-center gap-2 px-5 py-2 bg-cream text-black font-medium rounded-lg shadow-md hover:bg-white hover:text-black transition">Rp. 35.000</span>
									</button>
								</div>
							</div>
							<!--card product baru-->
							<div className="flex items-center mt-2.5 mb-5">
								<div className="flex items-center space-x-1 rtl:space-x-reverse">
								</div>
							</div>
						</div>
					</div>
				</div>
			</section>
			<!--Contact us dan chat popup-->
			<div>
				<div className="fixed bottom-4 right-2 z-50 ">
				  <button className="bg-teal text-white font-bold uppercase text-base px-5 py-3 rounded shadow-md hover:bg-white hover:text-black transition-all duration-300" onclick="toggleChat()">
					Contact Us
					</button>
				</div>
				   <div id="chat-popup" className="fixed bottom-8 right-2 bg-white border border-gray-300 rounded-lg shadow-lg w-80 mb-10 hidden z-50">
				       <div className="p-1">
								<h2 className="mb-0 pb-0 text-lg font-bold text-center text-gray-800">Welcome to COHEE</h2>
								<p className="mt-1 text-center text-gray-600">Ada yang bisa kami bantu?</p>
									<div className="mt-4 mb-4 flex flex-col">
											<form className="flex flex-col justify-center items-center " action="index.html" method="post">
												<input type="text" id="your_name" Placeholder="Your name"className="text-black h-8 px-4 py-0 mr-2 ml-2 mb-2 mt-2">
												<input type="email" id="your_email" Placeholder="you@gmail.com" className="text-black h-8 px-4 py-0 mr-2 ml-2 mb-2 mt-2">
												<input id="messageInput" type="text" placeholder="Tulis pesanmu.." className="text-black h-8 px-4 py-0 mr-2 ml-2 mb-2 mt-2"></input>
												<button id="sendButton" type="button" className=" bg-primary buttom-5 text-white xt-black h-8 px-4 py-0 hover:bg-white hover:text-black transition-all duration-300 ">Kirim</button>
												</form>
									</div>
					   	</div>
					</div>
			</div>
			</section>

			<!-- Footer -->
			<footer className="py-16 pb-8 bg-primary text-white">
				<div className="container mx-auto px-4 flex flex-col">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
						<div className="flex flex-col gap-4">
							<a href="#" className="block">
								<img
									src="./assets/images/coffee-logo.svg"
									alt="Cohee logo"
									className="w-[4.5rem] mb-4"
								/>
							</a>
							<p className="mb-6 leading-relaxed">
								Crafting exceptional coffee experiences since 2010. Join us in
								our passion for quality beans, expert brewing, and creating a
								warm, welcoming community space.
							</p>
							<div className="flex flex-col gap-3">
								<a
									href="#"
									className="text-white hover:text-secondary transition-all duration-300"
									>123 Eco Street, Green City</a
								>
								<a
									href="tel:+15555555555"
									className="text-white hover:text-secondary transition-all duration-300"
									>+1 (555) 555-5555</a
								>
								<a
									href="mailto:contact@comment.com"
									className="text-white hover:text-secondary transition-all duration-300"
									>contact@comment.com</a
								>
							</div>
						</div>
						<div>
							<h3 className="text-2xl mb-2">Quick Links</h3>
							<ul className="flex flex-col">
								<li className="my-2">
									<a
										href="index.html"
										className="text-white hover:text-secondary transition-all duration-300"
										>Home</a
									>
								</li>
								<li className="my-2">
									<a
										href="about.html"
										className="text-white hover:text-secondary transition-all duration-300"
										>About</a
									>
								</li>
								<li className="my-2">
									<a
										href="blogs.html"
										className="text-white hover:text-secondary transition-all duration-300"
										>Blog</a
									>
								</li>
							</ul>
						</div>
						<div>
							<h3 className="text-2xl mb-2">Social Media</h3>
							<ul className="flex flex-col">
								<li className="my-2">
									<a
										href="#"
										className="text-white hover:text-secondary transition-all duration-300"
									>
										<i className="fab fa-instagram"></i> Instagram
									</a>
								</li>
								<li className="my-2">
									<a
										href="#"
										className="text-white hover:text-secondary transition-all duration-300"
									>
										<i className="fab fa-facebook"></i> Facebook
									</a>
								</li>
								<li className="my-2">
									<a
										href="#"
										className="text-white hover:text-secondary transition-all duration-300"
									>
										<i className="fab fa-linkedin"></i> LinkedIn
									</a>
								</li>
							</ul>
						</div>
					</div>
					<hr className="my-6 border-none h-px bg-white/20" />
					<div className="text-center text-sm opacity-80">
						<p>Copyright &copy; 2025 Cohee. All rights reserved.</p>
					</div>
				</div>
			</footer>
		</main>

		<script src="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js"></script>
		<script>
			feather.replace();
		</script>
		<script src="./assets/js/script.js"></script>
		<script src="./assets/js/hamburger.js"></script>
		<script>
        function toggleChat() {
            const chatPopup = document.getElementById('chat-popup');
            chatPopup.classNameList.toggle('hidden');
        }
    </script>
		<script>
		     document.getElementById('sendButton').addEventListener('click', function() {
		         var message = document.getElementById('messageInput').value;
		         if (message) {
		             // Send the message (you can replace this with your actual send logic)
		             console.log('Message sent:', message);
		             // Clear the input box
		             document.getElementById('messageInput').value = '';
								 document.getElementById('your_email').value = '';
								 document.getElementById('your_name').value = '';
		         }
		     });
		 </script>
	</body>
</html>
